...
import com.example.addresslist.ItemAdapter;
...
public class AZTitleDecoration extends RecyclerView.ItemDecoration {
    ...
    public void onDraw(Canvas c, RecyclerView parent, RecyclerView.State state) {
        ...
        if (parent.getAdapter() == null || !(parent.getAdapter() instanceof ItemAdapter)) {
            return;
        }
        ItemAdapter adapter = (ItemAdapter) parent.getAdapter();
        if (adapter.getmPersonList() == null || adapter.getmPersonList().isEmpty()) {
            return;
        }
    ...
    private boolean titleAttachView(View view, RecyclerView parent) {
        ...
        return position == 0 ||
                null != adapter.getmPersonList().get(position) && !adapter.getSortLetters(position).equals(adapter.getSortLetters(position - 1));
